...
public class PlayerState : MonoBehaviour
{
    ...
    private bool _isDead; // guard, чтобы не сработала вторая смерть

    // Edge-triggered flags: remark fires only on the falling edge
    // of the threshold (not every frame the player is at low HP).
    // Reset on the rising edge so a healed player can re-trigger.
    private bool _wasMidHP; // < 50% (the 'mid' threshold, used to be 'lowHP' before round 88 v17)
    private bool _wasLowHP; // < 10% (the new 'really low' threshold)
    ...
    public void SetState()
    {
        ...
        // midHP (< 50%): edge-triggered. Fires once on the falling
        // edge of 50%, resets on the rising edge (back to 50%+).
        // Guard health > 0 ensures it does not re-arm during death.
        bool isMidHP = _info.health < 50f && _info.health > 0f;
        if (isMidHP && !_wasMidHP)
        {
            _dialog.Remarks.StartRemark(EnumData.RemarksType.midHP);
            _wasMidHP = true;
        }
        else if (!isMidHP && _wasMidHP)
        {
            _wasMidHP = false;
        }

        // lowHP (< 10%): same edge-triggered pattern. The
        // 'really low' alarm - separate from midHP because the
        // game has two distinct states: 'slightly wounded' (50%)
        // and 'about to die' (10%). midHP fires first, lowHP
        // fires later when the player ignores the first warning.
        // The 'health > 0' guard on both prevents firing on
        // death (where SetState is called one more time after
        // the die mode is entered).
        bool isLowHP = _info.health < 10f && _info.health > 0f;
